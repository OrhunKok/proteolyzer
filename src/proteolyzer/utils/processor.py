...
def _protease_rules(protease: str):
    ...
    rules = getattr(CONFIG.Protease, protease, None)
    if rules is None or not hasattr(rules, "ALLOWED_COUNTS"):
        available = sorted(
            name
            for name in dir(CONFIG.Protease)
            if not name.startswith("_")
            and hasattr(getattr(CONFIG.Protease, name), "ALLOWED_COUNTS")
        )
        raise ValueError(f"Invalid protease: '{protease}'. Must be one of: {available}")
    return rules
...
class DataProcessor(Logged):
    ...
    def miscleavages(
        ...
        rules = _protease_rules(protease).ALLOWED_COUNTS
