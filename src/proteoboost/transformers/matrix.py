...
class MatrixBuilder(metaclass=MetaLogging):
    ...
    def _missingness_check(self, matrix : pd.DataFrame, warning_threshold : float = 0.75) -> None:
        ...
        if not 0 <= warning_threshold <= 1:
            self.logger.error(f"warning_threshold must be between 0 and 1. Ignoring further missingness checks...")
        else:
            replaced_mnar = matrix.replace(0, np.nan)
            recommend_drop = replaced_mnar.apply(lambda x: x.isna().sum() / len(x)) > warning_threshold
            recommend_drop = list(recommend_drop[recommend_drop == True].index)
            if len(recommend_drop) > 0:
                self.logger.warning(f'Missingness over {round(warning_threshold * 100)}% for {recommend_drop}, recommend dropping these.')

    def matrix_generation(self, values: str, index: str, columns: str) -> pd.DataFrame:

        duplicate_count = self.data.duplicated(subset=[index, columns], keep=False).sum()
        
        if duplicate_count > 0:
            raise ValueError(f"{duplicate_count} Duplicate combinations of '{index}' and '{columns}' found. Matrix cannot be created.")
        else:
