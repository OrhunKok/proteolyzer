...
class DataProcessor(metaclass=MetaLogging):
    ...
    __slots__ = ('data_loader', 'data', 'INPUT_TYPE', 'logger', 'ID_COL', 'LABEL_GROUP_CAPTURE', 'LABEL_FREE')

    def __init__(self, data_loader: DataLoader, ID_COL: str = 'Precursor.Id', LABEL_GROUP_CAPTURE: str = r'\((?!.*\bUniMod\b)(.*?)\)'):
    ...
    def _check_labelfree(self) -> None:
        ...
        with warnings.catch_warnings():
            ...
            extracted_matches = self.data[self.ID_COL].str.contains(self.LABEL_GROUP_CAPTURE, regex=True).any()

        if extracted_matches:
            self.LABEL_FREE = False
            self.logger.info("Data appears to be labelled. Proceeding with generating labelling information...")
        else:
            self.LABEL_FREE = True
            self.logger.info("No labelling groups found in data. Data appears to be label-free.")

    def drop_identical_cols(self, df: pd.DataFrame) -> pd.DataFrame:
        """Drops columns with identical values."""
        cols_to_drop = [col for col in df.columns if df[col].nunique() == 1]
        if cols_to_drop:
            self.logger.info(f"Columns dropped for having identical values in all rows: {cols_to_drop}.")
    ...
    def convert_float_columns_to_int(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in float_cols:
            ...
            rounded_column_vals = np.round(pd.to_numeric(df[col], errors='coerce')).astype('Int64')
            if np.allclose(columns_vals, rounded_column_vals, equal_nan=True) or col_median > Constants.COL_MEDIAN_THRESHOLD:
                converted_cols += [col]
                df[col] = np.round(pd.to_numeric(df[col], errors='coerce')).astype('Int64')
    ...
    def convert_columns_to_categorical(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        cat_cols = list(cardinality[cardinality < Constants.CARDINALITY_THRESHOLD].index)
    ...
    def extra_info(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        transformations = {
            'Leading.Razor.Protein': (['Protein.Group'], lambda x: x.str.split(';').str[0]),
            'Peptide.Length': (['Stripped.Sequence'], lambda x: x.str.len()),
            'Label.Free': (['Stripped.Sequence', 'Precursor.Charge'], lambda x, y: x + y.astype(str)),
    ...
    def miscleavages(self, df: pd.DataFrame, seq_col: str = 'Stripped.Sequence', protease: str = 'Trypsin') -> pd.DataFrame:
        """Calculates miscleavages and adds a boolean column."""
        if protease not in Constants.PROTEASE_RULES:
            raise ValueError(f"Invalid protease: '{protease}'. Must be one of: {list(Constants.PROTEASE_RULES.keys())}")
        ...
        self.logger.info(f'{round(sum(bool_arrays) / len(bool_arrays) * 100, 1)}% Miscleavage rate according to {protease} rules')
...
class _LabelGenerator(metaclass=MetaLogging):
    ...
    __slots__ = (
        'data', 'ID_COL', 'LABEL_GROUP_CAPTURE', 'extracted_matches', 'sorted_matches',
        'UNIQUE_LABELS', 'UNIQUE_CHANNELS', 'logger'
    )
    ...
    def __init__(self, processed_data: ProcessedData):
        ...
        self.extracted_matches = self.data[self.ID_COL].str.extractall(self.LABEL_GROUP_CAPTURE)
        self.UNIQUE_LABELS = sorted(self.extracted_matches[0].str.split('-').str[0].unique())
    ...
    def _validate_matrix_shape(self, matrix: pd.DataFrame) -> pd.DataFrame:
        ...
        if matrix is None or not matrix.index.is_unique:
            self.logger.error(f'Label matrix is not the expected shape, requires custom manipulation! Skipping labelling info generation...')
            return None
    ...
    def _label_matrix(self, sorted_matches: pd.DataFrame) -> pd.DataFrame:
        ...
        for label in self.UNIQUE_LABELS:
            ...
            label_matrix = label_subset.pivot_table(index=['Index'], columns=['Location', 'Offset'], aggfunc='size').fillna(0).astype(int)
            label_matrix.columns = label_matrix.columns.map(''.join)
            name_matrix = np.char.multiply(np.array(label_matrix.columns, dtype=str), label_matrix.values)
            label_ids = [''.join(row) for row in name_matrix.tolist()]
            output_matrix.append(pd.DataFrame(label_ids, columns=[label + '.Label'], index=label_matrix.index, dtype='category'))
    ...
    def _label_counts(self, sorted_matches: pd.DataFrame) -> pd.DataFrame:
        ...
        label_count = pd.pivot(label_count, index='Index', columns='Count', values='Size').astype('Int8')

        return self._validate_matrix_shape(label_count)

    def _label_offset(self, sorted_matches: pd.DataFrame, label_count: pd.DataFrame) -> pd.DataFrame:
        """Generates the label offset matrix."""
        sorted_matches['Offset'] = sorted_matches['Offset'].astype(int)
        offset_sum = sorted_matches.pivot_table(index='Index', columns='Label', values='Offset', aggfunc='sum')
        offset_sum.columns = [col + '.Channel' for col in offset_sum.columns]

        if np.all(offset_sum.columns.str.split('.').str[0] == label_count.columns.str.split('.').str[0]):
            ...
            if np.allclose(offsets, rounded_offsets, equal_nan=True):
                ...
                self.logger.error(f'Channel offsets not uniform on peptide, cannot assign channel!')
                return None
        else:
            self.logger.error(f'Label count and Label offset dfs do not have the same column order, cannot assign channel!')
            return None
    ...
    def _generate_sorted_matches(self) -> pd.DataFrame:
        ...
        sorted_matches = self.extracted_matches[0].str.split('-', expand=True).reset_index()
        sorted_matches.columns = ['Index', 'Match', 'Label', 'Location', 'Offset']
        return sorted_matches

    def _add_label_info(self, df: pd.DataFrame, sorted_matches: pd.DataFrame) -> pd.DataFrame:
    ...
    def _generate_run_channels(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for label in self.UNIQUE_LABELS:
            ...
            if label_column in df.columns:
                df[f'Run.{label}.Channel'] = df['Run'].astype(str) + '-' + df[label_column].astype(str)
                df[f'Run.{label}.Channel'] = df[f'Run.{label}.Channel'].astype('category')

        label_columns = [f"{label}.Channel" for label in self.UNIQUE_LABELS if f"{label}.Channel" in df.columns]
        df['Run.Full.Channel'] = df['Run'].astype(str) + "-" + df[label_columns].astype(str).agg('-'.join, axis=1)
