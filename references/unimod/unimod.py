...
class UnimodDBLoader:
    ...
    DB_FILENAME: str = 'unimod.db' 
    DB_CONN: str = f"sqlite:///{DB_FILENAME}"
    ...
    def __init__(self, xsd_source: Optional[str] = None, xml_source: Optional[str] = None, db_conn: Optional[str] = None) -> None:
        ...
        if db_conn and connection_string.startswith("sqlite:///"):
            self.DB_FILENAME = connection_string.split("sqlite:///", 1)[1]
...
def main():
    ...
    parser.add_argument(
        '--db-conn', 
        type=str, 
        default=UnimodDBLoader.DB_CONN, 
        help=f"SQLAlchemy connection string (e.g., 'sqlite:///mydata.db'). Default: {UnimodDBLoader.DB_CONN}"
    )
