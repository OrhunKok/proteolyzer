...
pip install '.[unimod]'       # UniMod database tooling
...
matrix = (
    pz.transformers.MatrixBuilder(processed)
...
### UniMod: rebuild the reference tables

```bash
proteolyzer-unimod load --db-output unimod.db
proteolyzer-unimod process --db-file unimod.db
```

The CSVs this writes are what ship in `proteolyzer/resources` and back
`proteolyzer.config.AminoAcids`.

## Project layout

```
src/proteolyzer/
    config.py         reference data shared by every subpackage
    utils/            loading, processing and typed models (the core pipeline)
    transformers/     matrix construction and normalization
    plots/            plotting base class and relational plots
    cellenone/        cellenONE export parsing and well mapping
    unimod/           UniMod download, export and CLI
    aas/              amino acid substitution pipeline
        params.py     parameter file schema and loader
        base.py       shared stage plumbing and run provenance
        io.py         parquet interchange between stages
